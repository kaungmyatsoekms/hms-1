...
class HMSCashierFolio(models.Model):
    ...
    def _default_journal(self):
        ...
        if self._context.get('default_currency_id'):
            currency_clause = [('currency_id', '=',
                                self._context.get('default_currency_id'))]
            journal_with_currency = self.env['account.journal'].search(
                domain + currency_clause, limit=1)
        return journal_with_currency or self.env['account.journal'].search(
            domain, limit=1)
    ...
    def _get_payments_vals(self):
        ...
        for payment in self.payment_move_line_ids:
            ...
            if payment_currency_id and payment_currency_id == self.currency_id:
                ...
                amount_to_show = currency._convert(
                    amount, self.currency_id, payment.company_id, self.date
    ...
    move_name = fields.Char(
        ...
        copy=False,
        help=
        "Technical field holding the number given to the invoice, automatically set when the invoice is validated then stored to set the same number again if the invoice is cancelled, set to draft and re-validated."
    )
    ...
    date_due = fields.Date(
        ...
        copy=False,
        help=
        "If you use payment terms, the due date will be computed automatically at the generation "
    ...
    partner_id = fields.Many2one(
        ...
        track_visibility='always',
        help=
        "You can find a contact by its Name, TIN, Email or Internal Reference."
    )
    vendor_bill_id = fields.Many2one('hms.cashier.folio',
                                     string='Vendor Bill',
    ...
    tax_line_ids = fields.One2many('account.invoice.tax',
                                   ...
                                   states={'draft': [('readonly', False)]},
                                   copy=True)
    refund_invoice_ids = fields.One2many('hms.cashier.folio',
                                         'refund_invoice_id',
    ...
    journal_id = fields.Many2one(
        ...
        domain=
        "[('type', 'in', {'out_invoice': ['sale'], 'out_refund': ['sale'], 'in_refund': ['purchase'], 'in_invoice': ['purchase']}.get(type, [])), ('company_id', '=', company_id)]"
    )
    company_id = fields.Many2one('res.company',
                                 ...
                                 _company_default_get('hms.cashier.folio'))
    ...
    partner_bank_id = fields.Many2one(
        ...
        readonly=True,
        states={'draft': [('readonly', False)]
                })  #Default value computed in default_get for out_invoices
    ...
    payment_ids = fields.Many2many('account.payment',
                                   ...
                                   copy=False,
                                   readonly=True)
    payment_move_line_ids = fields.Many2many('account.move.line',
    ...
    def _get_computed_reference(self):
        ...
        if self.company_id.invoice_reference_type == 'invoice_number':
            ...
            prefix = 'CUST'
        return '%s/%s' % (prefix, str(identification_number % 97).rjust(
            2, '0'))
    ...
    def _onchange_vendor_bill(self):
        ...
        new_lines = self.env['hms.cashier.folio.line']
    ...
    def _get_seq_number_next_stuff(self):
        ...
        if self.journal_id.refund_sequence:
            ...
            journal_sequence = self.type in [
                'in_refund', 'out_refund'
            ] and self.journal_id.refund_sequence_id or self.journal_id.sequence_id
    ...
    def _compute_access_url(self):
        super(HMSCashierFolio, self)._compute_access_url()
        for invoice in self:
    ...
    def _get_sequence_prefix(self):
        ...
        for invoice in self:
            ...
            if (invoice.state == 'draft') and not self.search(domain, limit=1):
                prefix, dummy = journal_sequence.with_context(
                    ir_sequence_date=invoice.date_invoice,
    ...
    def _get_sequence_number_next(self):
        ...
        for invoice in self:
            journal_sequence, domain = invoice._get_seq_number_next_stuff()
            if (invoice.state == 'draft') and not self.search(domain, limit=1):
                number_next = journal_sequence._get_current_sequence(
                ).number_next_actual
    ...
    def _set_sequence_next(self):
        ...
        ) or not self.sequence_number_next or self.search_count(domain):
            return
    ...
    def create(self, vals):
        if not vals.get('journal_id') and vals.get('type'):
            vals['journal_id'] = self.with_context(
                type=vals.get('type'))._default_journal().id
    ...
    def validate_partner_bank_id(self):
        for record in self:
            if record.partner_bank_id:
                ...
                ) and record.partner_bank_id.partner_id != record.partner_id.commercial_partner_id:
                    raise ValidationError(
                        _("Commercial partner and vendor account owners must be identical."
                          ))
                elif record.type in (
                        'out_invoice', 'in_refund'
                ) and not record.company_id in record.partner_bank_id.partner_id.ref_company_ids:
                    raise ValidationError(
    ...
    def _write(self, vals):
        pre_not_reconciled = self.filtered(
            lambda invoice: not invoice.reconciled)
        pre_reconciled = self - pre_not_reconciled
        res = super(HMSCashierFolio, self)._write(vals)
        reconciled = self.filtered(lambda invoice: invoice.reconciled)
        not_reconciled = self - reconciled
        (reconciled & pre_reconciled).filtered(
            lambda invoice: invoice.state == 'open').action_invoice_paid()
        (not_reconciled
         & pre_not_reconciled).filtered(lambda invoice: invoice.state in (
             'in_payment', 'paid')).action_invoice_re_open()
        return res
    ...
    def default_get(self, default_fields):
        ...
        """
        res = super(HMSCashierFolio, self).default_get(default_fields)

        if res.get('type',
                   False) not in ('out_invoice',
                                  'in_refund') or not 'company_id' in res:
            return res
    ...
    def fields_view_get(self,
        ...
        def get_view_id(xid, name):
            try:
                ...
                view = self.env['ir.ui.view'].search([('name', '=', name)],
                                                     limit=1)
                if not view:
                    return False
                return view.id
        ...
        supplier_form_view_id = get_view_id(
            'invoice_supplier_form', 'hms.cashier.folio.supplier.form').id
        if context.get('active_model') == 'res.partner' and context.get(
                'active_ids'):
            partner = self.env['res.partner'].browse(context['active_ids'])[0]
            if not view_type:
                view_id = get_view_id('invoice_tree', 'hms.cashier.folio.tree')
                view_type = 'tree'
            elif view_type == 'form':
                if partner.supplier and not partner.customer:
                    view_id = supplier_form_view_id
                elif partner.customer and not partner.supplier:
                    view_id = get_view_id('invoice_form',
                                          'hms.cashier.folio.form').id

        return super(hms.cashier.folio,
    ...
    def action_invoice_sent(self):
        ...
        compose_form = self.env.ref('account.account_invoice_send_wizard_form',
                                    False)
        ctx = dict(default_model='account.invoice',
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        subscribed_emails = email_split((msg_dict.get('from') or '') + ',' +
                                        (msg_dict.get('cc') or ''))
        seen_partner_ids = [
            pid for pid in self._find_partner_from_emails(subscribed_emails)
            if pid
        ]
        ...
        partner_id = self._search_on_partner(email_from,
                                             extra_domain=[('supplier', '=',
                                                            True)])
        ...
        if not partner_id:
            ...
            if user_partner_id and user_partner_id in self.env.ref(
                    'base.group_user').users.mapped('partner_id').ids:
                ...
                if email_addresses:
                    partner_ids = [
                        pid for pid in self._find_partner_from_emails(
                            [email_addresses[0]], force_create=False) if pid
                    ]
        ...
        destination_emails = email_split((msg_dict.get('to') or '') + ',' +
                                         (msg_dict.get('cc') or ''))
        ...
        invoice = super(
            HMSCashierFolio,
            self.with_context(type=values.get('type'))).message_new(
                msg_dict, values)

        # Subscribe internal users on the newly created bill
        partners = self.env['res.partner'].browse(seen_partner_ids)
        is_internal = lambda p: (p.user_ids and all(
            p.user_ids.mapped(lambda u: u.has_group('base.group_user'))))
    ...
    def complete_empty_list_help(self):
        ...
        journals = Journal.browse(
            self._context.get('default_journal_id')) or Journal.search(
                [('type', '=', 'purchase')])
        ...
        if journals:
            ...
            for journal in journals.filtered(
                ...
                email = format(journal.alias_id.alias_name) + "@" + format(
                    journal.alias_domain)
                links += "<a id='o_mail_test' href='mailto:{}'>{}</a>".format(
                    email, email) + ", "
                alias_count += 1
            if links and alias_count == 1:
                help_message = _(
                    'Or share the email %s to your vendors: bills will be created automatically upon mail reception.'
                ) % (links[:-2])
            elif links:
                help_message = _(
                    'Or share the emails %s to your vendors: bills will be created automatically upon mail reception.'
                ) % (links[:-2])
            else:
                help_message = _(
                    '''Or set an <a data-oe-id=%s data-oe-model="account.journal" href=#id=%s&model=account.journal>email alias</a> '''
                    '''to allow draft vendor bills to be created upon reception of an email.'''
    ...
    def _onchange_cash_rounding(self):
        ...
        if self.cash_rounding_id and self.type in ('out_invoice',
                                                   'out_refund'):
            rounding_amount = self.cash_rounding_id.compute_difference(
                self.currency_id, self.amount_total)
            if not self.currency_id.is_zero(rounding_amount):
                if self.cash_rounding_id.strategy == 'biggest_tax':
                    ...
                    rounding_line = self.env['account.invoice.line'].new({
                        'name':
                        self.cash_rounding_id.name,
                        'invoice_id':
                        self.id,
                        'account_id':
                        self.cash_rounding_id.account_id.id,
                        'price_unit':
                        rounding_amount,
                        'quantity':
                        1,
        ...
        if report_invoice and report_invoice.attachment:
            for invoice in self:
                with invoice.env.do_in_draft():
                    ...
                    attachment = self.env.ref('account.account_invoices'
    ...
    def action_invoice_open(self):
        ...
        if to_open_invoices.filtered(lambda inv: float_compare(
                inv.amount_total, 0.0, precision_rounding=inv.currency_id.
                rounding) == -1):
            raise UserError(
                _("You cannot validate an invoice with a negative total amount. You should create a credit note instead."
                  ))
    ...
    def action_invoice_paid(self):
        ...
        if to_pay_invoices.filtered(lambda inv: inv.state not in
                                    ('open', 'in_payment')):
            raise UserError(
                _('Invoice must be validated in order to set it to register payment.'
                  ))
        ...
        for invoice in to_pay_invoices:
            if any([
                    move.journal_id.post_at_bank_rec and move.state == 'draft'
    ...
    def _notify_get_groups(self, message, groups):
        ...
        groups = super(HMSCashierFolio,
                       self)._notify_get_groups(message, groups)
    ...
    def _prepare_tax_line_vals(self, line, tax):
        ...
        vals = {
            ...
            'analytic_tag_ids':
            tax['analytic'] and line.analytic_tag_ids.ids or False,
        }
        ...
        if not vals.get(
                'account_analytic_id') and line.account_analytic_id and vals[
    ...
    def assign_outstanding_credit(self, credit_aml_id):
        ...
        if not credit_aml.currency_id and self.currency_id != self.company_id.currency_id:
            amount_currency = self.company_id.currency_id._convert(
                credit_aml.balance, self.currency_id, self.company_id,
                credit_aml.date or fields.Date.today())
            credit_aml.with_context(allow_amount_currency=True,
        ...
        for line in invoice_move_lines:
            if self.currency_id != company_currency:
                ...
                if not (line.get('currency_id')
                        and line.get('amount_currency')):
                    line['currency_id'] = currency.id
                    line['amount_currency'] = currency.round(line['price'])
                    line['price'] = currency._convert(line['price'],
                                                      company_currency,
                ...
                for i, t in enumerate(totlines):
                    if inv.currency_id != company_currency:
                        amount_currency = company_currency._convert(
                            t[1], inv.currency_id, inv.company_id,
                            inv._get_currency_rate_date()
                            or fields.Date.today())
                    else:
        ...
        for inv in self:
            if inv.cash_rounding_id:
                ...
                if rounding_amount != 0.0:
                    raise UserError(
                        _('The cash rounding cannot be computed because the difference must '
    ...
    def _check_duplicate_supplier_reference(self):
        for invoice in self:
            ...
            if invoice.type in ('in_invoice',
                                'in_refund') and invoice.reference:
    ...
    def invoice_validate(self):
        for invoice in self.filtered(lambda invoice: invoice.partner_id not in
    ...
    def line_get_convert(self, line, part):
        return self.env['product.product']._convert_prepared_anglosaxon_line(
            line, part)
    ...
    def name_get(self):
        ...
        for inv in self:
            result.append(
                (inv.id,
                 "%s %s" % (inv.number or TYPES[inv.type], inv.name or '')))
        return result
    ...
    def _name_search(self,
        ...
        invoice_ids = []
        if name:
            invoice_ids = self._search([('number', '=', name)] + args,
                                       limit=limit,
                                       access_rights_uid=name_get_uid)
        if not invoice_ids:
            invoice_ids = self._search([('name', operator, name)] + args,
                                       limit=limit,
                                       access_rights_uid=name_get_uid)
    ...
    def _refund_tax_lines_account_change(self, lines, taxes_to_change):
        ...
        for line in lines:
            if isinstance(line[2],
                          dict) and line[2]['tax_id'] in taxes_to_change:
                line[2]['account_id'] = taxes_to_change[line[2]['tax_id']]
        return lines

    def _get_refund_common_fields(self):
        return [
            'partner_id', 'payment_term_id', 'account_id', 'currency_id',
            'journal_id'
        ]
    ...
    def _get_refund_modify_read_fields(self):
        read_fields = [
            'type', 'number', 'invoice_line_ids', 'tax_line_ids', 'date'
        ]
        return self._get_refund_common_fields(
        ) + self._get_refund_prepare_fields() + read_fields

    @api.model
    def _get_refund_copy_fields(self):
        copy_fields = ['company_id', 'user_id', 'fiscal_position_id']
        return self._get_refund_common_fields(
        ) + self._get_refund_prepare_fields() + copy_fields
    ...
    def _prepare_refund(self,
        ...
        taxes_to_change = {
            ...
            for line in tax_lines.filtered(
                lambda l: l.tax_id.refund_account_id != l.tax_id.account_id)
        }
    ...
    def _prepare_payment_vals(self,
                              ...
                              writeoff_acc=None,
                              communication=None):
        payment_type = self.type in ('out_invoice',
                                     'in_refund') and 'inbound' or 'outbound'
        ...
        if not communication:
            communication = self.type in (
                'in_invoice', 'in_refund') and self.reference or self.number
            if self.origin:
        ...
        payment_vals = {
            ...
            self.partner_id.id,
            'partner_type':
            self.type in ('out_invoice', 'out_refund') and 'customer'
            or 'supplier',
            'journal_id':
            pay_journal.id,
            'payment_type':
            ...
            writeoff_acc and 'reconcile' or 'open',
            'writeoff_account_id':
            writeoff_acc and writeoff_acc.id or False,
        }
        return payment_vals
    ...
    def pay_and_reconcile(self,
        ...
        payment_vals = self._prepare_payment_vals(pay_journal,
    ...
    def _track_subtype(self, init_values):
        self.ensure_one()
        if 'state' in init_values and self.state == 'paid' and self.type in (
                'out_invoice', 'out_refund'):
            return 'account.mt_invoice_paid'
        elif 'state' in init_values and self.state == 'open' and self.type in (
                'out_invoice', 'out_refund'):
            return 'account.mt_invoice_validated'
        elif 'state' in init_values and self.state == 'draft' and self.type in (
                'out_invoice', 'out_refund'):
    ...
    def _get_onchange_create(self):
        return OrderedDict([
            ('_onchange_partner_id', [
                'account_id', 'payment_term_id', 'fiscal_position_id',
                'partner_bank_id'
            ]),
...
class HMSCashierFolioLine(models.Model):
    ...
    price_subtotal_signed = fields.Monetary(
        ...
        compute='_compute_price',
        help=
        "Total amount in the currency of the company, negative for credit note."
    )
    ...
    def _set_taxes(self):
        ...
        if self.invoice_id.type in ('out_invoice', 'out_refund'):
            ...
            ) or self.account_id.tax_ids or self.invoice_id.company_id.account_sale_tax_id
        else:
            taxes = self.product_id.supplier_taxes_id.filtered(
                lambda r: r.company_id == company_id
            ) or self.account_id.tax_ids or self.invoice_id.company_id.account_purchase_tax_id
        ...
        if self.invoice_id.type in ('in_invoice', 'in_refund'):
            ...
            if not self.price_unit or float_compare(
                ...
                self.price_unit = fix_price(self.product_id.standard_price,
                                            taxes, fp_taxes)
                self._set_currency()
        else:
    ...
    def _onchange_product_id(self):
        ...
        if not self.product_id:
            ...
            account = self.get_invoice_line_account(type, product, fpos,
                                                    company)
            if account:
    ...
    def _onchange_account_id(self):
        ...
        if not self.product_id:
            fpos = self.invoice_id.fiscal_position_id
            default_tax = self.invoice_id.type in (
                'out_invoice', 'out_refund'
            ) and self.invoice_id.company_id.account_sale_tax_id or self.invoice_id.company_id.account_purchase_tax_id
            self.invoice_line_tax_ids = fpos.map_tax(
                self.account_id.tax_ids or default_tax,
        ...
        if self.product_id and self.uom_id:
            ...
            if self.product_id.uom_id.category_id.id != self.uom_id.category_id.id:
                warning = {
                    ...
                    _('Warning!'),
                    'message':
                    _('The selected unit of measure has to be in the same category as the product unit of measure.'
                      ),
                }
    ...
    def unlink(self):
        if self.filtered(
                lambda r: r.invoice_id and r.invoice_id.state != 'draft'):
            raise UserError(
                _('You can only delete an invoice line if the invoice is in draft state.'
                  ))
        return super(HMSCashierFolioLine, self).unlink()
    ...
    def create(self, vals_list):
        ...
        return super(HMSCashierFolioLine, self).create(vals_list)
    ...
    def write(self, values):
        if 'display_type' in values and self.filtered(
            ...
            raise UserError(
                "You cannot change the type of an invoice line. Instead you should delete the current line and create a new line of the proper type."
            )
        return super(HMSCashierFolioLine, self).write(values)
        ...
        for tax in self:
            ...
            if tax.tax_id:
                key = tax.tax_id.get_grouping_key({
                    ...
                    tax.account_id.id,
                    'account_analytic_id':
                    tax.account_analytic_id.id,
                    'analytic_tag_ids':
                    tax.analytic_tag_ids.ids or False,
                })
...
class AccountPaymentTerm(models.Model):
    ...
    @api.multi
    def unlink(self):
        if self.env['account.invoice'].search([('payment_term_id', 'in',
                                                self.ids)]):
            raise UserError(
...
class AccountPaymentTermLine(models.Model):
    ...
    value = fields.Selection(
        [('balance', 'Balance'), ('percent', 'Percent'),
         ('fixed', 'Fixed Amount')],
        string='Type',
        required=True,
        default='balance',
    ...
    day_of_the_month = fields.Integer(
        ...
        help=
        "Day of the month on which the invoice must come to its term. If zero or negative, this value will be ignored, and no specific day will be set. If greater than the last day of a month, this number will instead select the last day of this month."
    )
    ...
    def _check_percent(self):
        if self.value == 'percent' and (self.value_amount < 0.0
                                        or self.value_amount > 100.0):
            raise ValidationError(
    ...
    def _check_days(self):
        if self.option in ('day_following_month',
                           'day_current_month') and self.days <= 0:
            raise ValidationError(
