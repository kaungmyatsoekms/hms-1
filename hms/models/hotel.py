...
class Property(models.Model):
    ...
    def write(self,values):
        ...
        if 'roomtype_ids' in values.keys():
            ...
            avail_objs = self.env['availability.availability'].search([('property_id','=', self.id)])
            for avail_obj in avail_objs:
                vals = []
                for roomtype in roomtypes:
                    ravail_obj = avail_obj.avail_roomtype_ids.filtered(lambda x: x.ravail_rmty.id == roomtype.id)
                    property_id = avail_obj.property_id.id
                    avail_date = avail_obj.avail_date
                    if not ravail_obj:
                        vals.append((0,0,{'ravail_rmty':roomtype.id,
                        'property_id':property_id,
                        'ravail_date':avail_date}))
