...
class HMSCashierFolio(models.Model):
    ...
    def _onchange_invoice_vendor_bill(self):
        if self.invoice_vendor_bill_id:
            ...
            for line in self.invoice_vendor_bill_id.invoice_line_ids:
                ...
                new_line = self.env['hms.cashier.folio.line'].new(copied_vals)
    ...
    def _get_tax_grouping_key_from_tax_line(self, tax_line):
        ...
        :param tax_line:    An hms.cashier.folio.line being a tax line (with 'tax_repartition_line_id' set then).
    ...
    def _get_tax_grouping_key_from_base_line(self, base_line, tax_vals):
        ...
        :param base_line:   An hms.cashier.folio.line being a base line (that could contains something in 'tax_ids').
    ...
    def _recompute_tax_lines(self, recompute_tax_base_amount=False):
        ...
        def _compute_base_line_taxes(base_line):
            ...
            :param base_line:   The hms.cashier.folio.line owning the taxes.
        ...
        # ==== Add tax lines ====
        to_remove = self.env['hms.cashier.folio.line']
    ...
    def _move_autocomplete_invoice_lines_create(self, vals_list):
        ''' During the create of an hms.cashier.folio with only 'invoice_line_ids' set and not 'line_ids', this method is called
    ...
    def _move_autocomplete_invoice_lines_write(self, vals):
        ''' During the write of an hms.cashier.folio with only 'invoice_line_ids' set and not 'line_ids', this method is called
        to auto compute accounting lines of the invoice. In that case, accounts will be retrieved and taxes, cash rounding
        and payment terms will be computed. At the end, the values will contains all accounting lines in 'line_ids'
        and the moves should be balanced.
    ...
    def _creation_subtype(self):
        ...
        if self.type in ('out_invoice', 'out_refund', 'out_receipt'):
            ...
            return super(HMSCashierFolio, self)._creation_subtype()
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        ''' Reverse a recordset of hms.cashier.folio.
        If cancel parameter is true, the reconcilable or liquidity lines
        of each original move will be reconciled with its reverse's.

        :param default_values_list: A list of default values to consider per move.
                                    ('type' & 'reversed_entry_id' are computed in the method).
        :return:                    An hms.cashier.folio recordset, reverse of the current self.
        '''
    ...
    def button_draft(self):
        AccountMoveLine = self.env['hms.cashier.folio.line']
        excluded_move_ids = []
    ...
    def action_invoice_register_payment(self):
        return self.env['account.payment']\
            .with_context(active_ids=self.ids, active_model='hms.cashier.folio', active_id=self.id)\
...
class HMSCashierFolioLine(models.Model):
    ...
    def write(self, vals):
        ...
        for line in self:
            ...
            result |= super(HMSCashierFolioLine, line).write(cleaned_vals)
    ...
    def unlink(self):
        ...
        res = super(HMSCashierFolioLine, self).unlink()
    ...
    def default_get(self, default_fields):
        # OVERRIDE
        values = super(HMSCashierFolioLine, self).default_get(default_fields)
    ...
    def _create_writeoff(self, writeoff_vals):
        """ Create a writeoff move per journal for the hms.cashier.folio.lines in self. If debit/credit is not specified in vals,
            ...
            :param writeoff_vals: list of dicts containing values suitable for account_move_line.create(). The data in vals will
                be processed to create bot writeoff hms.cashier.folio.line and their enclosing hms.cashier.folio.
        """
        ...
        writeoff_moves = self.env['hms.cashier.folio']
    ...
    def _get_matched_percentage(self):
        ...
        This is actually computing the same as the matched_percentage field of hms.cashier.folio, except in case of multi-currencies
    ...
    def _prepare_analytic_line(self):
        """ Prepare the values used to create() an account.analytic.line upon validation of an hms.cashier.folio.line having
