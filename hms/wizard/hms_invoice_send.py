...
class HmsInvoiceSend(models.TransientModel):
    _name = 'hms.invoice.send'
    _inherits = {'mail.compose.message':'composer_id'}
    _description = 'Hms Invoice Send'
    ...
    invoice_ids = fields.Many2many('hms.cashier.folio', 'hms_cashier_folio_hms_invoice_send_rel', string='Invoices')
    composer_id = fields.Many2one('mail.compose.message', string='Composer', required=True, ondelete='cascade')
    template_id = fields.Many2one(
        'mail.template', 'Use template', index=True,
        domain="[('model', '=', 'hms.cashier.folio')]"
        )

    @api.model
    def default_get(self, fields):
        res = super(HmsInvoiceSend, self).default_get(fields)
        res_ids = self._context.get('active_ids')

        invoices = self.env['hms.cashier.folio'].browse(res_ids).filtered(lambda move: move.is_invoice(include_receipts=True))
        if not invoices:
    ...
    def _compute_invoice_without_email(self):
        for wizard in self:
            if wizard.is_email and len(wizard.invoice_ids) > 1:
                invoices = self.env['hms.cashier.folio'].search([
