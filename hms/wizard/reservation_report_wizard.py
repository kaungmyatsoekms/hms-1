...
class ExpectedArrReportWizard(models.TransientModel):
    ...
    def get_report_excel(self):
        ...
        output = io.BytesIO()
        filename = 'Expected Arrival Report (' + str(
            self.property_id.code) + ') .xls'
        ...
        for obj in reservation_line_objs:
            row += 1
            worksheet.write(row, 0, obj.room_no.room_no or '',
                            for_left_not_bold)
            worksheet.write(row, 1, obj.room_type.code or '',
                            for_left_not_bold)
            ...
            worksheet.write(row, 7, obj.company_id.name or '',
                            for_left_not_bold)
            worksheet.write(row, 8, obj.arrival or '', for_left_not_bold)
            worksheet.write(row, 9, obj.arrival_flight or '',
                            for_left_not_bold)
            worksheet.write(row, 10, obj.arrival_flighttime or '',
                            for_left_not_bold)
            worksheet.write(row, 11, obj.departure or '', for_left_not_bold)
            worksheet.write(row, 12, obj.market.market_code or '',
                            for_left_not_bold)
            worksheet.write(row, 13, obj.source.source_code or '',
                            for_left_not_bold)
            worksheet.write(row, 14, obj.ratehead_id.rate_code or '',
                            for_left_not_bold)
        ...
        report_id = self.env['hms.excel.extended'].create({
            'excel_file': out,
            'file_name': filename
        })
        output.close()
