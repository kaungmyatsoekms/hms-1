        ...
        if startdate and enddate:
            ...
            if datetime.strptime(
                ...
                raise ValidationError(
                    _('Check-in date should be greater than or equal \
                                         the current date.'))
            if startdate > enddate:
                raise ValidationError(
                    _('Check-out date should be greater \
                                         than Check-in date.'))
...
class ReservationLine(models.Model):
    ...
    market = fields.Many2one('market.segment',
                             string="Market",
                             readonly=False,
                             related='reservation_id.market')
    source = fields.Many2one('market.source',
                             string="Source",
                             readonly=False,
                             related='reservation_id.source')
    reservation_type = fields.Many2one(
        'rsvn.type',
        "Reservation Type",
        readonly=True,
        related='reservation_id.reservation_type')
    reservation_status = fields.Many2one(
        'rsvn.status',
        "Reservation Status",
        readonly=False,
        related='reservation_id.reservation_status')
    arrival_flight = fields.Char("Arrival Flight",
                                 readonly=False,
                                 related='reservation_id.arrival_flight')
    arrival_flighttime = fields.Float(
        "AR_Flight Time",
        readonly=False,
        related='reservation_id.arrival_flighttime')
    dep_flight = fields.Char("Departure Flight",
                             readonly=False,
                             related='reservation_id.dep_flight')
    dep_flighttime = fields.Float("DEP_Flight Time",
                                  readonly=False,
                                  related='reservation_id.dep_flighttime')
